...
#        self.oDesign = self.oProject.SetActiveDesign()
    ...
    def integrate(self,frequency,phase):
        path = os.path.join(os.getcwd(),'phase.fld')
        _frequency = str(frequency)+"GHz"
        dr_x_name = self.getVariablevalue('dr_x')
        dr_x,unit = self.convertVariabletovalueandunit(dr_x_name)
        dr_y_name = self.getVariablevalue('dr_y')
        dr_y,unit = self.convertVariabletovalueandunit(dr_y_name)
        dr_z_name = self.getVariablevalue('dr_z')
        dr_z,unit = self.convertVariabletovalueandunit(dr_z_name)
        x = str(dr_x)+"mm"
        y = str(dr_y)+"mm"
        z = str(dr_z)+"mm"
    ...
    def export(self):
        ...
        oModule.ExportToFile("S Parameter Plot 1",path, False)
        cost = 0
        with open(path) as f:
            f_csv = csv.reader(f)
            headers_1 = next(f_csv)
